...
namespace FF1Lib
{
	public class TeleportShuffle
	{
		...
		public void LoadData()
		{
			...
			{
				e.Value.SetEntrance(new Coordinate(enter[(int)e.Key].X, enter[(int)e.Key].Y, CoordinateLocale.Standard));
			}

			foreach (var t in TeleMapping)
			{
				t.Value.SetEntrance(new Coordinate((byte)(tele[(int)t.Key].X | (t.Value.CoordinateX & 0x80)), tele[(int)t.Key].Y, CoordinateLocale.Standard));
			}
		}

		private Dictionary<OverworldTeleportIndex, TeleportDestination> EnterMapping => new Dictionary<OverworldTeleportIndex, TeleportDestination>
		{
		...
		private Dictionary<TeleportIndex, TeleportDestination> TeleMapping => new Dictionary<TeleportIndex, TeleportDestination>
		{
