...
namespace FF1Lib
{
	public partial class FF1Rom : NesRom
	{
		...
		private bool DoEnemizer_Enemies(MT19337 rng, EnemyInfo[] enemy, SpellInfo[] spell, EnemySkillInfo[] skill,
			EnemyScriptInfo[] script, string[] enemyNames, string[] skillNames, bool shuffledSkillsOn, EnemizerTrackingInfo en)
		{
			...
			List<byte> enemyImageLUT = new List<byte> { 0b00000000, 0b00000010, 0b00000001, 0b00000011, 0b00000100, 0b00000110, 0b00000101, 0b00000111,
														...
														0b00110000, 0b00110010, 0b00110001, 0b00110011 };
			...
			// we are reproducing the image array because we only want to shuffle the images, not the monsters themselves
			List<byte> enemyImages = new List<byte> { };
			for (int i = 0; i < Enemy.Lich; ++i)
			{
				enemyImages.Add(enemy[i].image);
			}

			// monster class modifiers that have effects on stats
			List<string>[] monsterClassVariants = InitializeMonsterClassVariants();
			// name variants for monsters to prevent duplicates
			List<string>[] monsterNameVariants = InitializeMonsterNameVariants();
			// true if the base name has been used, false if not
			bool[] monsterBaseNameUsed = InitializeMonsterBaseNameUsed();

			// shuffle the LUT - whatever image was there in vanilla will be replaced with what is at its position in the LUT
			enemyImageLUT.Shuffle(rng);
			// and shuffle the enemy images themselves
			enemyImages.Shuffle(rng);

			// generate the palettes for each tileset
			InitializeTilesetPalettes(rng, en);
			// clear the list of enemies in each tileset
			ClearEnemiesInTilesets(en);
			...
			{
				// list of perks this monster is eligible to receive
				List<MonsterPerks> perks = new List<MonsterPerks> { MonsterPerks.PERK_GAINSTAT10, MonsterPerks.PERK_LOSESTAT10 };

				// assign the monster's pic
				enemy[i].image = enemyImageLUT[enemyImages[i]];

				// enemy was outside the boundaries for acceptable tilesets - abort enemizer
				if (enemy[i].tileset >= GenericTilesetsCount)
				{
					return false;
				}

				// remember if enemy was small (false) or large (true)
				bool oldSize = enemy[i].Large;
				List<byte> acceptablepalettes = en.palettesInTileset[enemy[i].tileset].Except(enemyImagePalettes[enemy[i].image]).ToList();
				if (acceptablepalettes.Count == 0)
				{
					acceptablepalettes = en.palettesInTileset[enemy[i].tileset].ToList();
				...
				if (enemy[i].tier == -1)
				{
					...
					HandleSpecialMonster(i, enemy, enemyNames, newEnemyImageLUT);
				} else {
					// add enemy to enemiesInTileset unless it is a boss
					en.enemiesInTileset[enemy[i].tileset].Add(i);

					// adjust HP, EXP, and GP if enemy changes size
					AdjustStatsForSizeChange(enemy[i], oldSize);

					// track elemental affinity for certain classes of monsters (elementals and dragons)
					int elemental = 9;
					// default crit rate of 1 for most enemies
					enemy[i].critrate = 1;
					...
					ConfigureEnemyByImage(rng, i, enemy, enemyNames, newEnemyImageLUT, ref elemental, elementalsSelected, dragonsSelected);

					// generate perk eligibility
					AddEligiblePerks(perks);
					...
					bool hasGlobalPerk = ApplyGlobalPerk(i, enemy, rng);
					// false if this monster rolled a variant class and thus doesn't need a name modifier
					bool didntChooseVariantClass = true;
					...
					if (!hasGlobalPerk) {
						...
						didntChooseVariantClass = TryApplyClassVariant(rng, i, enemy, enemyNames, newEnemyImageLUT, monsterClassVariants, shuffledSkillsOn);
					}
					...
					if (didntChooseVariantClass)
					{
						...
						if (!hasGlobalPerk)
						{
							ApplyMinorPerks(rng, enemy[i], perks);
						}

						// if enemy's vanilla name has already been used, add a name modifier
						if (elemental == 9)
						{
							// don't make alternate names for elementals
							ApplyNameModifier(i, enemy, enemyNames, newEnemyImageLUT, monsterBaseNameUsed, monsterNameVariants, rng);
						}
					}
					FinalizeEnemyStats(rng, enemy[i], script, skill, spell);
				}
			}

			// remove palettes from tilesets where there are no mons using those palettes
			CleanupUnusedPalettes(en, enemy);
			...
			ModifyScripts(rng, script, spell);
		...
		private void HandleSpecialMonster(byte i, EnemyInfo[] enemy, string[] enemyNames, List<byte> newEnemyImageLUT)
		{
			switch (i)
			{
				case Enemy.Imp:
					...
					break;
				case Enemy.Pirate:
					// image forced to where the PIRATE image was moved
					enemy[i].image = (byte)newEnemyImageLUT.IndexOf(6);
					enemy[i].pal = 0x0B;
					break;
				case Enemy.Phantom:
					break;
				case Enemy.Garland:
					// image forced to where the GARLAND image was moved
					enemy[i].image = (byte)newEnemyImageLUT.IndexOf(46);
					enemy[i].pal = 0x13;
					break;
				case Enemy.Astos:
					// image forced to where the ASTOS image was moved
					enemy[i].image = (byte)newEnemyImageLUT.IndexOf(50);
					enemy[i].pal = 0x06;
					break;
				case Enemy.WarMech:
					enemyNames[i] = GetWarMechName(newEnemyImageLUT[enemy[i].image]);
					break;
			}
		}

		private string GetWarMechName(byte imageType)
		{
			var warMechNames = new Dictionary<byte, string>
			{
				{0, "WarIMP"}, {1, "WarAGAMA"}, {2, "WarWOLF"}, {3, "WarGIANT"},
				{4, "WarSAHAG"}, {5, "WarSHARK"}, {6, "WarBRUTE"}, {7, "WarEYE"},
				{8, "WarBONE"}, {9, "WarHYENA"}, {10, "WarCREEP"}, {11, "WarOGRE"},
				{12, "WarSNAKE"}, {13, "WarBULL"}, {14, "WarLBSTR"}, {15, "WarTROLL"},
				{16, "WarGHOST"}, {17, "WarWORM"}, {18, "WarGEIST"}, {19, "WarEYE"},
				{20, "WarLAMIA"}, {21, "WarPEDE"}, {22, "WarCAT"}, {23, "WarTIGER"},
				{24, "WarVAMP"}, {25, "WarDJINN"}, {26, "WarGOYLE"}, {27, "WarDRAKE"},
				{28, "WarOOZE"}, {29, "WarSPHNX"}, {30, "WarBUG"}, {31, "WarTURTL"},
				{32, "WarMUMMY"}, {33, "WarWYRM"}, {34, "WarBIRD"}, {35, "WarREX"},
				{36, "WarFISH"}, {37, "WarOCHO"}, {38, "WarGATOR"}, {39, "WarHYDRA"},
				{40, "WarMECH"}, {41, "WarNAGA"}, {42, "WarWATER"}, {43, "WarBEAST"},
				{44, "WarPISCO"}, {45, "WarDRAKE"}, {46, "WarGRLND"}, {47, "WarGOLEM"},
				{48, "WarMAN"}, {49, "WarPONY"}, {50, "WarELF"}, {51, "WarMECH"}
			};

			return warMechNames.TryGetValue(imageType, out string name) ? name : "WarMECH";
		}

		private void ConfigureEnemyByImage(MT19337 rng, byte i, EnemyInfo[] enemy, string[] enemyNames,
			List<byte> newEnemyImageLUT, ref int elemental, List<int> elementalsSelected, List<int> dragonsSelected)
		{
			byte imageType = newEnemyImageLUT[enemy[i].image];

			var config = GetEnemyConfiguration(imageType);
			enemyNames[i] = config.Name;
			enemy[i].monster_type = config.MonsterType;
			enemy[i].elem_resist = config.ElemResist;
			enemy[i].elem_weakness = config.ElemWeakness;

			if (config.CritRateRange.HasValue)
			{
				var (min, max) = config.CritRateRange.Value;
				enemy[i].critrate = rng.Between(min, max);
			}

			if (config.IsLargeElemental)
			{
				...
				enemyNames[i] = GetElementalName(elemental, true);
				if (elemental != 9)
				{
					elementalsSelected.Remove(elemental);
				}
			}
			else if (config.IsSmallElemental)
			{
				if (elementalsSelected.Count > 0)
				{
					elemental = elementalsSelected.PickRandom(rng);
				...
				enemyNames[i] = GetElementalName(elemental, false);
				if (elemental != 9)
				{
					elementalsSelected.Remove(elemental);
				}
			}
			else if (config.IsDragon)
			{
				...
				enemyNames[i] = GetDragonName(elemental, imageType == 27);
				if (elemental != 9)
				{
			...
			if (config.SpecialAbsorbChance)
			{
				// 50% chance of max absorb for this enemy type
			...
			if (config.SpecialGolemLogic)
			{
				ApplyGolemResistLogic(rng, enemy[i]);
			}

			if (config.SpecialFlanLogic)
			{
				ApplyFlanWeaknessLogic(rng, enemy[i]);
			}
		}

		private EnemizerVariant GetEnemyConfiguration(byte imageType)
		{
			var configs = new Dictionary<byte, EnemizerVariant>
			{
				// Imp
				{0, new EnemizerVariant("IMP", 0b00000100, 0, 0)},
				// Iguana
				{1, new EnemizerVariant("SAUR", 0b00000010, 0, 0)},
				// Wolf
				{2, new EnemizerVariant("WOLF", 0b00000000, 0, 0)},
				// Giant
				{3, new EnemizerVariant("GIANT", 0b00000100, 0, 0)},
				// Sahag
				{4, new EnemizerVariant("SAHAG", 0b00100000, 0b10010000, 0b01000000)},
				// Shark
				{5, new EnemizerVariant("SHARK", 0b00100000, 0b10010000, 0b01000000)},
				// Pirate
				{6, new EnemizerVariant("BRUTE", 0b00000000, 0b10000000, 0)},
				// Oddeye
				{7, new EnemizerVariant("EYES", 0b00100000, 0b10010000, 0b01000000)},
				// Bone
				{8, new EnemizerVariant("BONE", 0b00001000, 0b00101011, 0b00010000)},
				// Hyena
				{9, new EnemizerVariant("DOG", 0b00000000, 0, 0)},
				// Creep
				{10, new EnemizerVariant("CRAWL", 0b00000000, 0, 0b00010000)},
				// Ogre
				{11, new EnemizerVariant("OGRE", 0b00000100, 0, 0)},
				// Asp
				{12, new EnemizerVariant("ASP", 0b00000010, 0, 0) { CritRateRange = (1, 20) }},
				// Bull
				{13, new EnemizerVariant("BULL", 0b00000000, 0, 0)},
				// Crab
				{14, new EnemizerVariant("CRAB", 0b00000000, 0, 0)},
				// Troll
				{15, new EnemizerVariant("TROLL", 0b10000000, 0, 0)},
				// Spectral Undead / Ghost
				{16, new EnemizerVariant("GHOST", 0b00001001, 0b10101011, 0b00010000)},
				// Worm
				{17, new EnemizerVariant("WORM", 0b00000000, 0b10000000, 0)},
				// Zombie Undead / Wight
				{18, new EnemizerVariant("WIGHT", 0b00001000, 0b00101011, 0b00010000)},
				// Eye
				{19, new EnemizerVariant("EYE", 0b01000000, 0b10000000, 0)},
				// Medusa
				{20, new EnemizerVariant("LAMIA", 0b00000000, 0, 0)},
				// Pede
				{21, new EnemizerVariant("PEDE", 0b00000000, 0, 0)},
				// Were / Catman
				{22, new EnemizerVariant("CAT", 0b00010000, 0, 0)},
				// Tiger
				{23, new EnemizerVariant("TIGER", 0b00000000, 0, 0) { CritRateRange = (20, 80) }},
				// Vampire
				{24, new EnemizerVariant("VAMP", 0b10001001, 0b10101011, 0b00010000)},
				// Large Elemental
				{25, new EnemizerVariant("", 0, 0, 0) { IsLargeElemental = true }},
				// Gargoyle / Goyle
				{26, new EnemizerVariant("GOYLE", 0b00000001, 0b10000000, 0)},
				// Drake 1 / Dragon Type 1
				{27, new EnemizerVariant("", 0, 0, 0) { IsDragon = true }},
				// Slime / Flan
				{28, new EnemizerVariant("FLAN", 0b00000001, 0, 0) { SpecialFlanLogic = true, SpecialAbsorbChance = true }},
				// Manticore
				{29, new EnemizerVariant("MANT", 0b00000000, 0, 0)},
				// Spider
				{30, new EnemizerVariant("BUG", 0b00000000, 0, 0)},
				// Ankylo
				{31, new EnemizerVariant("ANK", 0b00000000, 0, 0)},
				// Mummy
				{32, new EnemizerVariant("MUMMY", 0b00001000, 0b00101011, 0b00010000)},
				// Wyvern
				{33, new EnemizerVariant("WYRM", 0b00000010, 0b10000000, 0)},
				// Jerk Bird / Bird
				{34, new EnemizerVariant("BIRD", 0b00000000, 0b10000000, 0)},
				// Steak / Tyro
				{35, new EnemizerVariant("TYRO", 0b00000010, 0, 0) { CritRateRange = (20, 80) }},
				// Pirahna / Caribe
				{36, new EnemizerVariant("FISH", 0b00100000, 0b10010000, 0b01000000)},
				// Ocho
				{37, new EnemizerVariant("OCHO", 0b00100000, 0b10010000, 0b01000000)},
				// Gator
				{38, new EnemizerVariant("GATOR", 0b00100010, 0b10010000, 0b01000000)},
				// Hydra
				{39, new EnemizerVariant("HYDRA", 0b00000010, 0, 0)},
				// Robot / Bot
				{40, new EnemizerVariant("BOT", 0b00000000, 0b00001011, 0b01000000)},
				// Naga
				{41, new EnemizerVariant("NAGA", 0b01000000, 0, 0)},
				// Small Elemental
				{42, new EnemizerVariant("AIR", 0, 0, 0) { IsSmallElemental = true }},
				// Chimera
				{43, new EnemizerVariant("BEAST", 0b00000010, 0b10010000, 0b00100000)},
				// Piscodemon
				{44, new EnemizerVariant("SQUID", 0b00000000, 0b00110011, 0)},
				// Dragon 2 / Dragon Type 2
				{45, new EnemizerVariant("", 0, 0, 0) { IsDragon = true }},
				// Knight Type 1
				{46, new EnemizerVariant("KNIGHT", 0b00000000, 0, 0)},
				// Golem
				{47, new EnemizerVariant("GOLEM", 0b00000000, 0b01111011, 0) { SpecialGolemLogic = true }},
				// Knight Type 2 / Badman
				{48, new EnemizerVariant("RANGER", 0b00000000, 0, 0)},
				// Pony
				{49, new EnemizerVariant("PONY", 0b00000000, 0, 0)},
				// Elf
				{50, new EnemizerVariant("ELF", 0b01000000, 0, 0)},
				// War Machine / Mech
				{51, new EnemizerVariant("MECH", 0b10000000, 0b00111011, 0b01000000)}
			};

			return configs.TryGetValue(imageType, out var config) ? config : new EnemizerVariant("UNKNOWN", 0, 0, 0);
		}

		private string GetElementalName(int elemental, bool isLarge)
		{
			var names = new Dictionary<int, (string large, string small)>
			{
				{1, ("EARTH", "SHARD")}, {2, ("STORM", "WIND")}, {3, ("ICE", "WATER")},
				{4, ("FIRE", "FLARE")}, {5, ("DEATH", "DOOM")}, {6, ("CRONO", "TIME")},
				{7, ("VENOM", "BANE")}, {8, ("DJINN", "MAGIC")}
			};

			if (names.TryGetValue(elemental, out var namePair))
			{
				return isLarge ? namePair.large : namePair.small;
			}

			return isLarge ? "FORCE" : "AIR";
		}

		private string GetDragonName(int elemental, bool isDrake)
		{
			var names = new Dictionary<int, string>
			{
				{1, "Earth D"}, {2, "Elec D"}, {3, "Ice D"}, {4, "Fire D"},
				{5, "Death D"}, {6, "Time D"}, {7, "Gas D"}, {8, "Magic D"}
			};

			if (names.TryGetValue(elemental, out var name))
			{
				return name;
			}

			return isDrake ? "DRAKE" : "DRAGON";
		}

		private void ApplyGolemResistLogic(MT19337 rng, EnemyInfo enemy)
		{
			if (enemy.absorb > (enemy.tier < 3 ? 30 : 60))
			{
				int resistToRemove = rng.Between(0, 2);
				byte mask = resistToRemove switch
				{
					0 => 0b10111111,
					1 => 0b11011111,
					_ => 0b11101111
				};
				enemy.elem_resist &= mask;
			}
		}

		private void ApplyFlanWeaknessLogic(MT19337 rng, EnemyInfo enemy)
		{
			// can be fire, ice, fire+ice, lit, fire+lit, or ice+lit weak
			enemy.elem_weakness = (byte)(rng.Between(1, 6) << 4);
			// resist all other elements except time
			enemy.elem_resist = (byte)((0b11111111 ^ enemy.elem_weakness) & 0b11111011);
		}

		private bool ApplyGlobalPerk(byte enemyIndex, EnemyInfo[] enemy, MT19337 rng)
		{
			if (enemyIndex == Enemy.Crawl)
			{
				// Global Perk - multi-hitter with weak attacks but stun touch guaranteed (overrides existing atk ailment)
				enemy[enemyIndex].atk_elem = 0b00000001;
				enemy[enemyIndex].atk_ailment = 0b00010000;
				enemy[enemyIndex].num_hits = 8;
				enemy[enemyIndex].damage = 1;
				return true;
			}

			if (enemyIndex == Enemy.Coctrice)
			{
				// Global Perk - stonetouch (overrides other atk_elem and ailment), -1 hit if num_hits > 1
				enemy[enemyIndex].atk_elem = 0b00000010;
				enemy[enemyIndex].atk_ailment = 0b00000010;
				if (enemy[enemyIndex].num_hits > 1)
				{
					enemy[enemyIndex].num_hits--;
				}
				return true;
			}

			if (enemyIndex == Enemy.Sorcerer)
			{
				// Global Perk - deathtouch
				enemy[enemyIndex].atk_elem = 0b00000000;
				enemy[enemyIndex].atk_ailment = 0b00000001;
				return true;
			}
		...
		private bool TryApplyClassVariant(MT19337 rng, byte i, EnemyInfo[] enemy, string[] enemyNames,
			List<byte> newEnemyImageLUT, List<string>[] monsterClassVariants, bool shuffledSkillsOn)
		{
			byte imageType = newEnemyImageLUT[enemy[i].image];
			var variants = monsterClassVariants[imageType];

			if (rng.Between(0, 11) >= variants.Count)
			{
				return true;
			}

			string classModifier = variants.PickRandom(rng);
			// apply the traits of this enemy class
			ApplyClassModifierEffects(classModifier, enemy[i], shuffledSkillsOn, rng);
			// change the enemy's name
			enemyNames[i] = classModifier + enemyNames[i];
			// remove this variant from the list of variants available for this enemy image
			variants.Remove(classModifier);

			return false;
		}

		private void ApplyClassModifierEffects(string modifier, EnemyInfo enemy, bool shuffledSkillsOn, MT19337 rng)
		{
			switch (modifier)
			{
				case "Wz":
					if (enemy.AIscript == 0xFF && !shuffledSkillsOn)
					{
						enemy.AIscript = ENE_PickForcedAIScript(enemy.tier, rng);
					}
					break;

				case "Fr":
					enemy.elem_resist |= 0b00100000;
					enemy.elem_resist &= 0b11101111;
					enemy.elem_weakness |= 0b00010000;
					enemy.elem_weakness &= 0b11011111;
					enemy.atk_elem = 0b00100000;
					break;

				case "R.":
					enemy.elem_resist |= 0b00010000;
					enemy.elem_resist &= 0b11011111;
					enemy.elem_weakness |= 0b00100000;
					enemy.elem_weakness &= 0b11101111;
					enemy.atk_elem = 0b00010000;
					break;

				case "Z.":
					enemy.elem_resist |= 0b00101011;
					enemy.elem_resist &= 0b11101111;
					enemy.elem_weakness |= 0b00010000;
					enemy.elem_weakness &= 0b11010100;
					enemy.monster_type |= 0b00001000;
					if (enemy.atk_ailment == 0)
					{
						enemy.atk_ailment = 0b00010000;
						enemy.atk_elem = 0b00000001;
					}
					break;

				case "Sea":
					enemy.elem_resist |= 0b10010000;
					enemy.elem_resist &= 0b10111111;
					enemy.elem_weakness |= 0b01000000;
					enemy.elem_weakness &= 0b01101111;
					enemy.monster_type |= 0b00100000;
					break;

				case "Wr":
					enemy.atk_ailment = 0b00000100;
					enemy.atk_elem = 0b00000010;
					enemy.monster_type |= 0b10010000;
					break;
			}
		}

		private void ApplyMinorPerks(MT19337 rng, EnemyInfo enemy, List<MonsterPerks> perks)
		{
			int numPerks = 0;
			while (rng.Between(0, 7) > 1 + numPerks && numPerks < 4)
			{
				// select from the list of available perks
				MonsterPerks perk = perks.PickRandom(rng);
				bool appliedSuccessfully = ApplySinglePerk(rng, enemy, perk);
		...
		private bool ApplySinglePerk(MT19337 rng, EnemyInfo enemy, MonsterPerks perk)
		{
			switch (perk)
			{
				case MonsterPerks.PERK_GAINSTAT10:
					return ApplyGainStatPerk(rng, enemy);

				case MonsterPerks.PERK_LOSESTAT10:
					return ApplyLoseStatPerk(rng, enemy);

				case MonsterPerks.PERK_LOWRESIST:
					return ApplyLowResistPerk(rng, enemy);

				case MonsterPerks.PERK_LOWWEAKNESS:
					return ApplyLowWeaknessPerk(rng, enemy);

				case MonsterPerks.PERK_HIGHRESIST:
					return ApplyHighResistPerk(rng, enemy);

				case MonsterPerks.PERK_HIGHWEAKNESS:
					return ApplyHighWeaknessPerk(rng, enemy);

				case MonsterPerks.PERK_PLUSONEHIT:
					return ApplyPlusOneHitPerk(enemy);

				case MonsterPerks.PERK_POISONTOUCH:
					return ApplyPoisonTouchPerk(enemy);

				case MonsterPerks.PERK_STUNSLEEPTOUCH:
					return ApplyStunSleepTouchPerk(rng, enemy);

				case MonsterPerks.PERK_MUTETOUCH:
					return ApplyMuteTouchPerk(enemy);
		...
		private bool ApplyGainStatPerk(MT19337 rng, EnemyInfo enemy)
		{
			// pick a random stat to increase by 10%, +3% XP
			int statChoice = rng.Between(0, 2);
			switch (statChoice)
			{
				case 0:
					enemy.hp = enemy.hp * 11 / 10;
					break;
				case 1:
					enemy.damage = enemy.damage * 11 / 10;
					break;
				case 2:
					enemy.absorb = enemy.absorb * 11 / 10;
					break;
			}
			enemy.exp = enemy.exp * 103 / 100;
			return true;
		}

		private bool ApplyLoseStatPerk(MT19337 rng, EnemyInfo enemy)
		{
			// pick a random stat to decrease by 10%, -3% XP
			int statChoice = rng.Between(0, 2);
			switch (statChoice)
			{
				case 0:
					enemy.hp = enemy.hp * 9 / 10;
					break;
				case 1:
					enemy.damage = enemy.damage * 9 / 10;
					break;
				case 2:
					enemy.absorb = enemy.absorb * 9 / 10;
					break;
			}
			enemy.exp = enemy.exp * 97 / 100;
			return true;
		}

		private bool ApplyLowResistPerk(MT19337 rng, EnemyInfo enemy)
		{
			// pick a low resist or remove the corresponding weakness, +4% XP
			var elementBits = new[] { 0b10000000, 0b01000000, 0b00100000, 0b00010000 };
			byte elementBit = (byte)elementBits[rng.Between(0, 3)];

			if ((enemy.elem_weakness & elementBit) == elementBit)
			{
				enemy.elem_weakness &= (byte)~elementBit;
			}
			else
			{
				enemy.elem_resist |= elementBit;
			}

			enemy.exp = enemy.exp * 104 / 100;
			return true;
		}

		private bool ApplyLowWeaknessPerk(MT19337 rng, EnemyInfo enemy)
		{
			// pick a low weakness (or cancel a resist, or ignore for earth resist), -5% XP
			var elementBits = new[] { 0b10000000, 0b01000000, 0b00100000, 0b00010000 };
			byte elementBit = (byte)elementBits[rng.Between(0, 3)];

			// Earth resist special case
			if ((enemy.elem_resist & elementBit) == elementBit && elementBit == 0b10000000)
			{
				return false;
			}

			if ((enemy.elem_resist & elementBit) == elementBit)
			{
				enemy.elem_resist &= (byte)~elementBit;
			}
			else
			{
				enemy.elem_weakness |= elementBit;
			}

			enemy.exp = enemy.exp * 95 / 100;
			return true;
		}

		private bool ApplyHighResistPerk(MT19337 rng, EnemyInfo enemy)
		{
			// pick a high resist and remove the corresponding weakness, +3% XP
			var resistAndWeaknessPairs = new[]
			{
				(resist: 0b00001000, weakness: 0b11110111),
				(resist: 0b00000100, weakness: 0b11111011),
				(resist: 0b00000010, weakness: 0b11111101),
				(resist: 0b00000001, weakness: 0b11111110)
			};

			var pair = resistAndWeaknessPairs[rng.Between(0, 3)];
			enemy.elem_resist |= (byte)pair.resist;
			enemy.elem_weakness &= (byte)pair.weakness;
			enemy.exp = enemy.exp * 103 / 100;
			return true;
		}

		private bool ApplyHighWeaknessPerk(MT19337 rng, EnemyInfo enemy)
		{
			// pick a high weakness (or cancel a resist), -3% XP
			var elementBits = new[] { 0b00001000, 0b00000100, 0b00000010, 0b00000001 };
			byte elementBit = (byte)elementBits[rng.Between(0, 3)];
		...
		private bool ApplyPlusOneHitPerk(EnemyInfo enemy)
		{
			// +1 hit, +5/3/2/1... XP for an addition hit
			enemy.num_hits++;

			int expMultiplier = enemy.num_hits switch
			{
				2 => 105,
				3 => 103,
				4 => 102,
			...
			enemy.exp = enemy.exp * expMultiplier / 100;
			return true;
		}

		private bool ApplyPoisonTouchPerk(EnemyInfo enemy)
		{
			// adds poisontouch if enemy has no atk_ailment already, no XP increase
			if (enemy.atk_ailment != 0)
			{
				return false;
			}
		...
		private bool ApplyStunSleepTouchPerk(MT19337 rng, EnemyInfo enemy)
		{
			// adds stun or sleep touch if enemy has no atk_ailment already, +3% XP
			if (enemy.atk_ailment != 0)
			{
				return false;
			}

			enemy.atk_ailment = rng.Between(0, 1) == 0 ? (byte)0b00100000 : (byte)0b00010000;
			enemy.atk_elem = 0b00000001;
			enemy.exp = enemy.exp * 103 / 100;
			return true;
		}

		private bool ApplyMuteTouchPerk(EnemyInfo enemy)
		{
			// adds mute touch if enemy has no atk_ailment already, +2% XP
			if (enemy.atk_ailment != 0)
			{
				return false;
			}

			enemy.atk_ailment = 0b01000000;
			enemy.atk_elem = 0b00000001;
			enemy.exp = enemy.exp * 102 / 100;
			return true;
		}

		private void ApplyNameModifier(byte i, EnemyInfo[] enemy, string[] enemyNames, List<byte> newEnemyImageLUT,
			bool[] monsterBaseNameUsed, List<string>[] monsterNameVariants, MT19337 rng)
		{
			byte imageType = newEnemyImageLUT[enemy[i].image];

			if (monsterBaseNameUsed[imageType])
			{
				string nameModifier = monsterNameVariants[imageType].PickRandom(rng);
				enemyNames[i] = nameModifier + enemyNames[i];
				monsterNameVariants[imageType].Remove(nameModifier);
			}
			else
			{
				monsterBaseNameUsed[imageType] = true;
			}
		}

		private void FinalizeEnemyStats(MT19337 rng, EnemyInfo enemy, EnemyScriptInfo[] script,
			EnemySkillInfo[] skill, SpellInfo[] spell)
		{
			// set monster type to Mage if it has a script
			if (enemy.AIscript != 0xFF)
			{
				enemy.monster_type |= 0b01000000;
			}

			// for each hit past the first, reduce the base damage by one for every 15 points of damage rating
			for (int j = 1; j < enemy.num_hits; ++j)
			{
				enemy.damage = enemy.damage - enemy.damage / 15;
			}

			// force death resist on undead enemies and remove weakness to death
			if ((enemy.monster_type & 0b00001000) != 0)
			{
				enemy.elem_resist |= 0b00001000;
				enemy.elem_weakness &= 0b11110111;
			}

			// variance for damage rating
			enemy.damage = rng.Between(enemy.damage - enemy.damage / 25, enemy.damage + enemy.damage / 25);
			// variance for hp rating
			enemy.hp = rng.Between(enemy.hp - enemy.hp / 30, enemy.hp + enemy.hp / 30);
			// variance for gp reward
			enemy.gp = rng.Between(enemy.gp - enemy.gp / 20, enemy.gp + enemy.gp / 20);
			// variance for exp reward
			enemy.exp = rng.Between(enemy.exp - enemy.exp / 40, enemy.exp + enemy.exp / 40);

			if (enemy.AIscript != 0xFF)
			{
				// determine skill tier
				enemy.skilltier = DetermineSkillTier(enemy, script, skill, spell);
			}
		}

		private int DetermineSkillTier(EnemyInfo enemy, EnemyScriptInfo[] script,
			EnemySkillInfo[] skill, SpellInfo[] spell)
		{
			int highestTier = 0;

			foreach (byte id in script[enemy.AIscript].skill_list)
			{
				if (id == 0xFF)
					continue;
			...
			foreach (byte id in script[enemy.AIscript].spell_list)
			{
				if (id == 0xFF)
					continue;
		...
		private void AdjustStatsForSizeChange(EnemyInfo enemy, bool oldSize)
		{
			if (oldSize && enemy.Small)
			{
				// large became small
				enemy.hp = enemy.hp * 4 / 5;
				enemy.exp = enemy.exp * 9 / 10;
				enemy.gp = enemy.gp * 9 / 10;
			}
			else if (!oldSize && enemy.Large)
			{
				// small became large
				enemy.hp = enemy.hp * 5 / 4;
				enemy.exp = enemy.exp * 10 / 9;
				enemy.gp = enemy.gp * 10 / 9;
			}
		}

		private void AddEligiblePerks(List<MonsterPerks> perks)
		{
		...
		private void CleanupUnusedPalettes(EnemizerTrackingInfo en, EnemyInfo[] enemy)
		{
			for (int i = 0; i < GenericTilesetsCount; ++i)
			{
				List<byte> palRemoveList = new List<byte>();
		...
		private void ModifyScripts(MT19337 rng, EnemyScriptInfo[] script, SpellInfo[] spell)
		{
			// exclude the last 10 scripts
			for (int i = 0; i < script.Length - 10; ++i)
			{
				...
				for (byte j = 0; j < 8; ++j)
				{
					...
					int tier = spell[script[i].spell_list[j]].tier;
					// tier 0 becomes tier 1
					if (tier == 0)
						tier = 1;
		...
		private void InitializeTilesetPalettes(MT19337 rng, EnemizerTrackingInfo en)
		{
			for (int i = 0; i < GenericTilesetsCount; ++i)
			{
				en.palettesInTileset[i].Clear();
		...
		private void ClearEnemiesInTilesets(EnemizerTrackingInfo en)
		{
			for (int i = 0; i < GenericTilesetsCount; ++i)
				en.enemiesInTileset[i].Clear();
		}

		private List<string>[] InitializeMonsterClassVariants()
		{
			var variants = new List<string>[52];

			// Imp
			variants[0] = new List<string> { "Fr", "R.", "Z.", "Wr", "Wz", "Sea" };
			// Iguana
			variants[1] = new List<string> { "Fr", "R.", "Z.", "Wz", "Sea" };
			// Wolf
			variants[2] = new List<string> { "Fr", "R.", "Z.", "Wr", "Wz" };
			// Giant
			variants[3] = new List<string> { "Fr", "R.", "Z.", "Wz", "Sea" };
			// Sahag
			variants[4] = new List<string> { "Fr", "Z.", "Wr", "Wz" };
			// Shark
			variants[5] = new List<string> { "Fr", "Z." };
			// Pirate
			variants[6] = new List<string> { "Fr", "R.", "Z.", "Wz" };
			// Oddeye
			variants[7] = new List<string> { "Fr", "Z." };
			// Bone
			variants[8] = new List<string> { "Wz" };
			// Hyena
			variants[9] = new List<string> { "Fr", "R.", "Z.", "Wz", "Sea" };
			// Creep
			variants[10] = new List<string> { "Fr", "R.", "Z.", "Wr", "Wz", "Sea" };
			// Ogre
			variants[11] = new List<string> { "Fr", "R.", "Z.", "Wr" };
			// Asp
			variants[12] = new List<string> { "Fr", "R.", "Z.", "Sea" };
			// Bull
			variants[13] = new List<string> { "Fr", "R.", "Z.", "Wr", "Wz", "Sea" };
			// Crab
			variants[14] = new List<string> { "Fr", "R.", "Z.", "Sea" };
			// Troll
			variants[15] = new List<string> { "Fr", "R.", "Z.", "Wz", "Sea" };
			// Spectral Undead / Ghost
			variants[16] = new List<string> { "Wz" };
			// Worm
			variants[17] = new List<string> { "Fr", "R.", "Z.", "Sea" };
			// Zombie Undead / Wight
			variants[18] = new List<string> { "Sea" };
			// Eye
			variants[19] = new List<string> { "Fr", "R.", "Z.", "Sea" };
			// Medusa
			variants[20] = new List<string> { "Fr", "R.", "Z.", "Wz", "Sea" };
			// Pede
			variants[21] = new List<string> { "Fr", "R.", "Z.", "Sea" };
			// Were / Catman
			variants[22] = new List<string> { "Fr", "R.", "Z.", "Wz" };
			// Tiger
			variants[23] = new List<string> { "Fr", "R.", "Z.", "Wr" };
			// Vampire
			variants[24] = new List<string> { "Wz" };
			// Large Elemental
			variants[25] = new List<string> { };
			// Gargoyle / Goyle
			variants[26] = new List<string> { "Fr", "R.", "Z.", "Wz", "Sea" };
			// Drake 1 / Dragon Type 1
			variants[27] = new List<string> { };
			// Slime / Flan
			variants[28] = new List<string> { "Wz" };
			// Manticore
			variants[29] = new List<string> { "Fr", "R.", "Z.", "Wz", "Sea" };
			// Spider
			variants[30] = new List<string> { "Fr", "R.", "Z.", "Sea" };
			// Ankylo
			variants[31] = new List<string> { "Fr", "R.", "Z." };
			// Mummy
			variants[32] = new List<string> { "Wz" };
			// Wyvern
			variants[33] = new List<string> { "Fr", "R.", "Z.", "Wr", "Wz" };
			// Jerk Bird / Bird
			variants[34] = new List<string> { "Fr", "R.", "Z." };
			// Steak / Tyro
			variants[35] = new List<string> { "Fr", "R.", "Z.", "Sea" };
			// Pirahna / Caribe
			variants[36] = new List<string> { "Fr", "Z." };
			// Ocho
			variants[37] = new List<string> { "Fr", "Z.", "Wz" };
			// Gator
			variants[38] = new List<string> { "Fr", "Z.", "Wr" };
			// Hydra
			variants[39] = new List<string> { "Fr", "R.", "Z.", "Sea" };
			// Robot / Bot
			variants[40] = new List<string> { };
			// Naga
			variants[41] = new List<string> { "Fr", "Z.", "Sea" };
			// Small Elemental
			variants[42] = new List<string> { };
			// Chimera
			variants[43] = new List<string> { "Z.", "Wr", "Wz" };
			// Piscodemon
			variants[44] = new List<string> { "Z.", "Wz" };
			// Dragon 2 / Dragon Type 2
			variants[45] = new List<string> { };
			// Knight Type 1
			variants[46] = new List<string> { "Fr", "R.", "Z.", "Wz" };
			// Golem
			variants[47] = new List<string> { };
			// Knight Type 2 / Badman
			variants[48] = new List<string> { "Fr", "R.", "Z.", "Wz" };
			// Pony
			variants[49] = new List<string> { "Fr", "R.", "Z.", "Wr", "Sea" };
			// Elf
			variants[50] = new List<string> { "Fr", "R.", "Z.", "Sea" };
			// War Machine / Mech
			variants[51] = new List<string> { };

			return variants;
		}

		private List<string>[] InitializeMonsterNameVariants()
		{
			var nameVariants = new List<string>[52];
			for (int i = 0; i < 52; ++i)
			{
				nameVariants[i] = new List<string> { "A.", "B.", "C.", "D.", "E.", "F.", "G.", "H.", "I.", "K.", "L.", "M.", "N.", "P.", "S.", "T.", "V.", "W.", "X." };
			}
			return nameVariants;
		}

		private bool[] InitializeMonsterBaseNameUsed()
		{
			var baseNameUsed = new bool[52];
			// if enemy is not part of a variant class, by default it uses the base name for the monster
			for (int i = 0; i < 52; ++i)
			{
				baseNameUsed[i] = false;
			}

			baseNameUsed[25] = true; // large elemental
			baseNameUsed[27] = true; // dragon 1
			baseNameUsed[42] = true; // small elemental
			baseNameUsed[45] = true; // dragon 2
