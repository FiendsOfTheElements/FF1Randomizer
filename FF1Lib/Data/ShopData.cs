...
namespace FF1Lib
{
	...
	public class ShopData
	{
		...
		private readonly Dictionary<int, Shop> ShopPrototypes = new()
		{
		...
		private readonly MemTable<ushort> Index;
		...
		public void LoadData()
		{
			...
			Shops = new List<Shop>();
			foreach (ShopType shopType in Enum.GetValues<ShopType>())
			{
		...
		public int StoreData()
		{
			...
			for (int i = 0; i < 70; i++)
			{
				if (shopdic.TryGetValue(i, out Shop shop))
				{
		...
		private List<Shop> GetShops(ShopType shopType)
		{
			...
			for (int i = 0; i < ShopSectionSize; i++)
			{
				...
				{
					Shop prototype = ShopPrototypes[(int)shopType + i];
