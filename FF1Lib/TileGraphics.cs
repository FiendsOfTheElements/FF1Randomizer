...
namespace FF1Lib
{
	public partial class FF1Rom
	{
		...
		public async Task ReplaceTileGraphics(TileSets tileSet, Stream imageStream)
		{
			int maxCHR = 128;
			...
			Image<Rgba32> tilesImage = Image.Load<Rgba32>(imageStream);
		...
		{
			FF1Rom rom;
			Dictionary<Rgba32, byte> outRoomPalette;
			Dictionary<Rgba32, byte> inRoomPalette; //rooms have their palettes inverted in rom
			Dictionary<Rgba32, byte> roomWallPalette; //compromise to allow all detail to be shown
			List<TileDetails> CastleTileReference;
			...
			public TileGraphicsRefData(FF1Rom rom)
			{
				...
				CastleTileReference = new List<TileDetails>
				{
					...
					new() {palette = outRoomPalette,  tilenums = new() {0x30} }, //wall U
					...
					new() {palette = outRoomPalette,  tilenums = new() {0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D, 0x2E,
						0x2F, 0x3E, 0x3F, 0x41, 0x42, 0x43, 0x46, 0x47, 0x4A, 0x4B, 0x56, 0x57, 0x79, 0x7A, 0x7B, 0x7C, 0x7D, 0x7E} }, //empty black
